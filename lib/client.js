...
var Client = function (config) {
	...
	if (!config.baseUrl) {
		...
		if (config.userId.includes("--TRIAL--")) {
			config.baseUrl = "https://odast02lt9.execute-api.us-east-1.amazonaws.com/bturner/app/safe-trials"
		}
	}
	...
	function createRequestOptions (params) {
		var baseUrl = config.baseUrl + (params.apiVersion ? "/" + params.apiVersion : apiVersionPath);
		var userPath = params.pathWithoutUser ? "" : (usersPath + "/" + (params.userId || config.userId));
		return {
